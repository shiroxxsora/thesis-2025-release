...
class ComprehensiveAnalyzer:
    ...
    def read_cadastral_data_enhanced(self, mif_path: str, raster_crs, raster_bounds=None) -> List[Dict]:
        ...
        self._crs_cadastre_same_as_raster = False
        try:
            driver = ogr.GetDriverByName("MapInfo File")
            datasource = driver.Open(mif_path, 0)
            if datasource is None:
            ...
            if source_srs_cadastr.IsSame(target_srs_raster):
                ...
                self._crs_cadastre_same_as_raster = True
            ...
            # GDAL автоматически выполняет правильную трансформацию координат
            # Ручное смещение НЕ ТРЕБУЕТСЯ - проверено в QGIS
            print(f"[INFO] GDAL выполнит автоматическую трансформацию из CRS кадастра в CRS растра", flush=True)
            ...
            for feature in layer:
                ...
                try:
                    ...
                    if geom_clone.GetGeometryType() == ogr.wkbPolygon:
                        shp = shape(json.loads(geom_clone.ExportToJson()))
                        if shp.is_valid and shp.area > 0:
                            # GDAL уже выполнил трансформацию, дополнительное смещение не нужно
                            if raster_bbox is None or shp.intersects(raster_bbox):
                                bounds = shp.bounds
            ...
            if sample_bounds:
                print(f"[DEBUG] Примеры bounds полигонов ПОСЛЕ трансформации и смещения (первые 5):", flush=True)
    ...
    def analyze_violations(self, detected_objects: List[Dict], cadastral_objects: List[Dict],
                          use_violation_offset_heuristic: bool = True) -> List[Dict]:
        ...
        # Эвристика ±4 млн по X: только если CRS кадастра и растра различались (зона vs без зоны).
        # При совпадающих CRS не применяем — иначе сдвиг при привязке нарушений к участкам.
        violation_coord_offset = 0.0
        if use_violation_offset_heuristic and detected_objects and cadastral_objects:
            ...
            if abs(x_diff - 4000000) < 100000 or abs(x_diff + 4000000) < 100000:
                if x_diff > 0:
                    violation_coord_offset = -4000000.0
                else:
                    violation_coord_offset = 4000000.0
    ...
    def run_analysis(self):
        ...
        use_heuristic = not getattr(self, '_crs_cadastre_same_as_raster', False)
        violations = self.analyze_violations(detected_objects, cadastral_objects, use_violation_offset_heuristic=use_heuristic)
